...
=========================
Prints a detailed diagnostic report each update to pinpoint:
  - Return normalization issues (scale collapse / explosion)
  - Unstable value predictions (explained variance, raw magnitude)
  - Policy collapse (entropy, log_std drift, KL divergence)
  - Gradient health (per-module norms, NaN fraction)
...
def compute_explained_variance(y_pred: np.ndarray, y_true: np.ndarray) -> float:
    ...
    EV ~ 1 means value function is accurate. EV ~ 0 means it predicts the mean.
    EV < 0 means it's worse than predicting the mean.
    """
...
def print_diagnostics(update: int, rollout_diag: dict, update_diag: dict):
    ...
    ud = update_diag
    lines.append(f"    log_std        : {_fmt_array(ud.get('log_std_values', None))}")
    ...
    lines.append(f"    value_loss     : {ud.get('value_loss', 0.0):>12.6f}  (×VF_COEF=0.5 → {ud.get('value_loss', 0.0)*0.5:>12.6f})")
    lines.append(f"    entropy_bonus  : {ud.get('entropy', 0.0):>12.6f}  (×ENT_COEF=0.01 → {ud.get('entropy', 0.0)*0.01:>12.6f})")
    ...
    lines.append(f"    grad_global_norm: {ud.get('grad_norm', 0.0):>10.6f}  "
                 f"(clipped to {0.5})")
    lines.append(f"    grad_nan_frac   : {ud.get('grad_nan_frac', 0.0):>10.6f}  "
                 f"{'<-- NaN GRADS!' if ud.get('grad_nan_frac', 0) > 0 else ''}")
    for module_name in ['cnn', 'proprio', 'trunk0', 'trunk1', 'actor', 'critic0', 'critic1']:
        key = f'grad_norm_{module_name}'
        if key in ud:
            lines.append(f"      {module_name:>10s}   : {ud[key]:>10.6f}")
    ...
    if ud.get('grad_nan_frac', 0) > 0:
        warnings.append(f"NaN gradients detected ({ud.get('grad_nan_frac', 0)*100:.2f}%)")
...
def _fmt_array(arr):
    """Format a small array or scalar for display."""
    if arr is None:
        return "N/A"
    arr = np.asarray(arr)
    if arr.ndim == 0:
    ...
    if arr.size <= 12:
        return "[" + ", ".join(f"{float(v):.3f}" for v in arr.flat) + "]"
    return f"mean={float(arr.mean()):.4f} std={float(arr.std()):.4f} min={float(arr.min()):.4f} max={float(arr.max()):.4f}"
